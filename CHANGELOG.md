...
- CSV join reports `error_code: missing_boundaries` with a curator-facing `message` when the `oeps/{level}-{year}` shapefile is missing (HTTP 403/404), instead of a raw S3 403. 2010 ZCTA: “2010 ZCTA boundaries aren’t in the library; use 2018 for ZCTA (including zcta-ruca-2010.csv).” Manager may append “contact us.”
