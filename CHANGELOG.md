...
- CSV join reports `error_code: missing_boundaries` with a curator-facing `message` when the `oeps/{level}-{year}` shapefile is missing (HTTP 403/404), instead of a raw S3 403. Example: “2010 ZCTA boundaries aren’t in the library.” Manager may append “contact us.” RAs filling 2010 vintages in `herop-geodata` removes the error without a Lambda change.
...
- Geo path (`upload_kind: geo`) also reads **GeoPackage** (`.gpkg`), same dissolve/simplify as GeoJSON / shapefile zip. Still no HEROP join. KML / file gdb stay rejected.
- Geo path ([#6](https://github.com/healthyregions/SDOHPlace-SpatialPipeline/issues/6)): `upload_kind: geo` reads GeoJSON, GeoPackage, or a shapefile zip, reprojects to EPSG:4326, simplifies, and writes WKT, `ENVELOPE(W,E,N,S)`, and centroid (`lat,lon`). `highlight_ids` and `spatial_coverage` are `[]` plus warnings (no HEROP join, no place-name matching). KML / file gdb are rejected.
- CSV ID join ([#7](https://github.com/healthyregions/SDOHPlace-SpatialPipeline/issues/7), [#9](https://github.com/healthyregions/SDOHPlace-SpatialPipeline/issues/9)): `upload_kind: csv` against 2010 or 2018 US Census shapefiles under `oeps/`. Prefers `FIPS`, then `geo_id_column` / `HEROP_ID` / `GEOID`; read as string; zfill so Excel-stripped leading zeros (e.g. `6037` → `06037`) still match `prefix + FIPS == HEROP_ID`. `spatial_level` is `bg` (not `blockgroup`). **2018** `highlight_ids` uses the coverage.py include / minus / `*` rules. **2010** returns geometry from 2010 units and `highlight_ids` `[]` (discovery tiles are 2018-only). Zero matches or mixed GEOID lengths fail. Partial matches succeed with diagnostics. `spatial_coverage` is unique **state names** from matched HEROP IDs (plus `United States` / `Contiguous US` when coverage is national / CONUS), not from place-name matching. Geometry is the simplified dissolve of **matched** units.
...
- Marynia (Sept 2026 Slack): RA uploads are shapefile, GeoJSON, or GeoPackage (geo path). Points already in those files are in scope. Geocoding is out of scope. CSV lat/long columns are an October extension, not this slice.
